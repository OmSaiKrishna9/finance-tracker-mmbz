...
console.log('🔧 BACKEND_URL from environment:', process.env.REACT_APP_BACKEND_URL);
                  ...
                  <>
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          value={editingItem.shoot_type}
                          onChange={(e) => handleEditChange('shoot_type', e.target.value)}
                          required
                          className="w-full px-4 py-2 border border-gray-300 rounded-lg"
                        />
                      </div>
                      <div>
                        ...
                        <input
                          type="number"
                          step="0.5"
                          value={editingItem.total_time_hrs}
                          onChange={(e) => handleEditChange('total_time_hrs', parseFloat(e.target.value))}
                          required
                          className="w-full px-4 py-2 border border-gray-300 rounded-lg"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          value={editingItem.received_by}
                          onChange={(e) => handleEditChange('received_by', e.target.value)}
                          required
                          className="w-full px-4 py-2 border border-gray-300 rounded-lg"
                        />
                      </div>
...
function AdminUserManagement() {
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${BACKEND_URL}/api/admin/users`, { withCredentials: true });
      setUsers(response.data);
    } catch (error) {
      console.error('Error fetching users:', error);
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingUser) {
        // Update
        await axios.put(`${BACKEND_URL}/api/admin/users/${editingUser.id}`, formData, { withCredentials: true });
      ...
      setShowModal(false);
      setEditingUser(null);
      setFormData({ name: '', email: '', mobile: '', role: 'EMPLOYEE' });
      fetchUsers();
    } catch (error) {
      console.error('Error saving user:', error);
      setToast({ message: error.response?.data?.detail || 'Failed to save user', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6" data-testid="admin-page">
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          >
            <span className="text-xl">+</span> Create User
          </button>
        </div>
